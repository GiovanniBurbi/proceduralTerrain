...
export const noise = (function() {
  ...
  class Noise {
    ...
    // computeBiome(x, y, width, coords, noiseFunc) {
    //   let amplitude = 1
    //   let frequency = 1
    //   let noiseHeight = 0
    ...
    generateNoiseMap(coords, width, dim) {
      ...
      for (let y = 0; y < width; y++){
        for(let x = 0; x < width; x++){  
          ...
          // heights[y * width + x] = Math.round((normHeight * this.params.terrain.maxHeight) * 100) / 100 
    ...
    //         normHeight = 0
    //       }
    //       heights[y * width + x] = Math.round((Math.pow(normHeight, this.params.noise.exponentiation)) * 100) / 100 
    //     }
    //   }
    //   return heights
    // }

    // generateBiomeMap(coords, width, dim){
    //   const biome = new Array(dim)
    //   const noiseFunc = this.noise[this.params.noise.type]
    ...
    //   for (let y = 0; y < width; y++){
    //     for(let x = 0; x < width; x++){
    //       biome[y * width + x] = this.computeBiome(x, y, width, coords, noiseFunc)
    //     }
    //   }
  
    //   for (let y = 0; y < width; y++){
    //     for(let x = 0; x < width; x++){  
    //       let normBiome = (biome[y * width + x] + 1) / (maxPossibleValue)
    //       if (normBiome < 0) {
    //         normBiome = 0
    //       }
    //       biome[y * width + x] = Math.round((Math.pow(normBiome, this.params.noise.exponentiation)) * 100) / 100
    //     }
    //   }
